...
export default class BattleHandler {
    constructor(battleContent, cb) {
        sceneHandler.openBattleScene(battleContent.stage, battleContent.enemies)
        const PlayerDataComponent = gameObjectsHandler.getGameObjectByName("player").getComponent(PlayerData)
        PlayerDataComponent.hidePlayerDataUI()


        // Temporary to ensure transition between world and battle scenes are all set up
        setTimeout(() => {
            // IMPORTANT:
            cb()
            PlayerDataComponent.showPlayerDataUI()
            PlayerDataComponent.updateXp(15)

            sceneHandler.returnToWorldScene()
        }, 3000)
        ...
        // gsap.to(this.container, {autoAlpha: 1, pointerEvents: "auto"})
    ...
    loop() {
        if (this._phase !== "endBattle") {
            ...
            gsap.to(this.container, {autoAlpha: 0, pointerEvents: "none"})
            // gsap.to(document.querySelector(".log"), {autoAlpha: 0, pointerEvents: "none"})
        }
    }
}
